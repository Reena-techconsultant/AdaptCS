...
const CookiePolicyDialog: React.FC = () => {
  const [accepted, setAccepted] = useState<boolean>(false);
  const [rejected, setRejected] = useState<boolean>(false);

  useEffect(() => {
    const isCookiePolicyAccepted = localStorage.getItem("cookiePolicyAccepted");
    const isCookiePolicyRejected = localStorage.getItem("cookiePolicyRejected");

    if (isCookiePolicyAccepted === "true") {
      setAccepted(true);
    } else if (isCookiePolicyRejected === "true") {
      setRejected(true);
    }
  }, []);
  ...
  const handleReject = () => {
    localStorage.setItem("cookiePolicyRejected", "true");
    setRejected(true);
  };
  ...
  return (
    <div className="fixed bottom-0 right-0 z-[60] sm:max-w-xl w-full mx-auto p-6">
      {/* Card */}
      <div className="p-4  border  rounded-xl shadow-sm bg-gray-800 border-gray-700">
        <div className="flex gap-x-5">
          ...
          >
            ...
            <mask
              ...
              fill="currentColor"
              className="text-gray-800 fill-gray-200"
            >
              <ellipse
                rx="1.09811"
                ry="0.738034"
          ...
          <div className="grow">
            ...
            <div className="mt-5 inline-flex gap-x-2">
              <button
                type="button"
                className="py-2 px-3 inline-flex justify-center items-center gap-2 rounded-md border border-transparent font-semibold bg-blue-500 text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all text-sm focus:ring-offset-gray-800"
